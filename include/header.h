...
typedef struct
{
  ...
  char* s_class; 
  char* s_data;
  char* s_version_ident; 
  char* s_osabi;
  char* s_abiversion; 
  char* s_type; 
  char* s_machine; 
  char* s_version; // e_version
...
char* get_elf_version_ident(unsigned char* e_ident);
...
char* get_section_header_string_table_index(uint64_t shstrndx);
